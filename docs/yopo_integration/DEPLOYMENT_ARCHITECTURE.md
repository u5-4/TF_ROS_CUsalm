...
冲突风险。禁止为了减少启动终端而把这些环境重新合并。
    ...
    ├── YOPO runtime（当前为宿主机 Conda 环境 yopo）
    │   ├── /home/nvidia/catkin_ws/src/YOPO_ROS2
    │   ├── /home/nvidia/miniconda3/envs/yopo
    │   ├── YOPO Python / PyTorch
    │   └── 轨迹候选输出
...
| 运行时 | 拥有 | 不得拥有 |
| --- | --- | --- |
| localization-runtime | D435I、Isaac ROS、NITROS、cuVSLAM、定位适配、标准深度和状态输出 | YOPO/PyTorch 环境、规划决策、SO3、解锁和模式切换 |
| YOPO runtime | 宿主机 Conda 环境 yopo、YOPO 模型、目标输入、轨迹生成 | D435I 驱动、cuVSLAM、MAVROS、PX4 external vision、解锁 |
...
跨运行时只交换 ROS 2 消息。localization 容器不得挂载或复用 YOPO 的 Conda
site-packages；YOPO 环境也不得复用 localization 容器中的 CUDA wheel 或动态库。
...
实际 topic 名称在接口实现阶段固定。上述方向不表示当前已经授权 YOPO 或控制输出。
...
| YOPO runtime | tools/collect_yopo_runtime_baseline.sh | 宿主机 /home/nvidia/catkin_ws/src/YOPO_ROS2，Conda yopo |
