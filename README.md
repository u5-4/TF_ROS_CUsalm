...
- `bag_contract_probe` C++17 只读离线审计工具，用于固定的 30 分钟双源 rosbag；
- raw/shadow、IMU relay、cuVSLAM status/odometry、timesync 和 diagnostics 合同统计；
- 不创建 `/localization/odometry`、`/state/odom`、TF 或任何 `/mavros/*` publisher。

本分支已经实现传输、时间戳和运行合同统计，但尚未实现 source selector、cuVSLAM/动捕一次性对齐或 MAVROS gateway。由于 `T[base_link,camera_link]`、`T[mocap_world,odom]` 和 VRPN 光学捕获时刻尚未批准，跨源轨迹误差仍为 `NOT_AUTHORIZED`；工具不会输出 ATE、RPE、轴向/偏航误差或飞行可用性结论。Jetson 编译和离线报告复核通过前，不得把本节状态改成可飞行。
